...
files <- list.files(
  path = "//trdecomsrv/H/CRS Reports/RP-500 Compensating Tax/Industry Unredacted/",
  pattern = "\\.xls$|\\.xlsx$",
  full.names = TRUE
)
...
RP500COMPInd <- sapply(files, read_excel, simplify = FALSE) %>% bind_rows(.id = "id")

#delete columns that are blank
RP500COMPInd$id <- NULL
RP500COMPInd$...4 <- NULL
RP500COMPInd$...5 <- NULL
RP500COMPInd$...6 <- NULL
RP500COMPInd$...7 <- NULL
RP500COMPInd$...9 <- NULL
RP500COMPInd$...11 <- NULL
RP500COMPInd$...17 <- NULL

#rename the columns
names(RP500COMPInd)[names(RP500COMPInd) == "New Mexico"] <- "County"
names(RP500COMPInd)[names(RP500COMPInd) == "...2"] <- "Industry"
names(RP500COMPInd)[names(RP500COMPInd) == "...3"] <- "Type"
names(RP500COMPInd)[names(RP500COMPInd) == "...8"] <- "Returns"
names(RP500COMPInd)[names(RP500COMPInd) == "...10"] <- "TotalGR"
names(RP500COMPInd)[names(RP500COMPInd) == "...12"] <- "TaxableGR"
names(RP500COMPInd)[names(RP500COMPInd) == "...13"] <- "MatchedTGR"
names(RP500COMPInd)[names(RP500COMPInd) == "...14"] <- "TaxDue"
names(RP500COMPInd)[names(RP500COMPInd) == "...15"] <- "Taxpaid"
names(RP500COMPInd)[names(RP500COMPInd) == "...18"] <- "RecipientDue"
names(RP500COMPInd)[names(RP500COMPInd) == "...16"] <- "RecipientPaid"
names(RP500COMPInd)[names(RP500COMPInd) == "...19"] <- "Adjustment"

#fill in missing values
RP500COMPInd <- fill(RP500COMPInd, County, Industry, .direction = c("down"))

RP500 <- subset(RP500COMPInd, `County`!="Taxation and Revenue" & `County`!="Local Government Distribution" 
                & `County`!="by Industry" & `County`!="Revenue Group:"
...
Activity <- RP500 %>%
  mutate(
    Activity = case_when(
      RP500$TotalGR %in% formatted_dates_5 ~ as.character(RP500$TotalGR %in% formatted_dates_6),
    )
  )  
...
RP500COMPInd <- Activity[c("Activity", "County", "Location", "Industry", "Type", "Returns", "TotalGR", "TaxableGR", "MatchedTGR", "TaxDue", "Taxpaid", "RecipientDue", "RecipientPaid", "Adjustment")]

#format values
RP500COMPInd$Returns <- as.numeric(RP500COMPInd$Returns)
RP500COMPInd$TotalGR <- as.numeric(RP500COMPInd$TotalGR)
RP500COMPInd$TaxableGR <- as.numeric(RP500COMPInd$TaxableGR)
RP500COMPInd$MatchedTGR <- as.numeric(RP500COMPInd$MatchedTGR)
RP500COMPInd$TaxDue <- as.numeric(RP500COMPInd$TaxDue)
RP500COMPInd$Taxpaid <- as.numeric(RP500COMPInd$Taxpaid)
RP500COMPInd$RecipientDue <- as.numeric(RP500COMPInd$RecipientDue)
RP500COMPInd$RecipientPaid <- as.numeric(RP500COMPInd$RecipientPaid)

#################################################################################################################################################################################################
# Export datasets

#save as R file
save(RP500COMPInd, file = "//trdecomsrv/H/CRS Reports/R Backup Databases/RP500 COMP Industry/RP500COMPIndustry_Alphie.RData")
#save to STATA
require(foreign)
write.dta(RP500COMPInd, "//trdecomsrv/H/CRS Reports/STATA Databases/RP500COMPInd_Alphie.dta")
# Save to SAS
write.foreign(df = RP500COMPInd,
              datafile = '//trdecomsrv/H/CRS Reports/R Backup Databases/RP500 COMP Industry/RP500COMPInd_Alphie.RData',
              codefile = '//trdecomsrv/H/CRS Reports/R Backup Databases/RP500 COMP Industry/RP500COMPInd_Alphie.sas',
              package = 'SAS')
